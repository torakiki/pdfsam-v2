...
public class IcePdfThumbnailsCreator extends AbstractThumbnailCreator {
	...
	protected boolean openInputDocument() throws ThumbnailCreationException {
		...
		if (inputFile!=null && inputFile.exists() && inputFile.isFile()){
			try {
				...
				throw new ThumbnailCreationException(GettextResource.gettext(Configuration.getInstance().getI18nResourceBundle(), "Error opening pdf document")+ " " + inputFile.getAbsolutePath(), e);
			} 
		}else{
	 ...
	 * @throws IOException
	 * @throws NullPointerException if the inputFile is null
	 */
	private Document openDocument(final File inputFile, final String providedPwd) throws PDFException, PDFSecurityException, IOException, NullPointerException {
	...
	public BufferedImage getPageImage(File fileName, String password, int page, int rotation)throws ThumbnailCreationException {
		...
		if(fileName != null && fileName.exists() && fileName.isFile()){
			try {
				...
				retVal = (BufferedImage)pdfDocument.getPageImage(page-1,GraphicsRenderingHints.SCREEN,Page.BOUNDARY_CROPBOX, rotation, 1);
			} catch (Exception e) {
				throw new ThumbnailCreationException(GettextResource.gettext(Configuration.getInstance().getI18nResourceBundle(), "Error opening pdf document")+ " " + fileName.getAbsolutePath(), e);
			} 
		}else{
	...
	public BufferedImage getThumbnail(File inputFile, String password, int page, float resizePercentage) throws ThumbnailCreationException {
		...
		if(inputFile != null && inputFile.exists() && inputFile.isFile()){
			try {
				...
				retVal = (BufferedImage)pdfDocument.getPageImage(page-1,GraphicsRenderingHints.SCREEN,Page.BOUNDARY_CROPBOX, 0, resizePercentage);
			} catch (Exception e) {
				throw new ThumbnailCreationException(GettextResource.gettext(Configuration.getInstance().getI18nResourceBundle(), "Error opening pdf document")+ " " + inputFile.getAbsolutePath(), e);
			} 
		}else{
