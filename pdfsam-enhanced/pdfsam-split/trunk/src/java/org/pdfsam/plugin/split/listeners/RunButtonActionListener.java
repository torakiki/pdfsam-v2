			...
			if(items != null && items.length == 1){
                ...
                if (splitType.equals(SplitParsedCommand.S_SPLIT)){
                    ...
                    if(panel.getSplitSizeCombo().isSelectedItem() && panel.getSplitSizeCombo().isValidSelectedItem()){
                    	...
                    	throw new Exception(GettextResource.gettext(Configuration.getInstance().getI18nResourceBundle(),"Invalid split size"));
                    }
                ...
                if (panel.getSameAsSourceRadio().isSelected()){
                	...
                	if(panel.getDestinationFolderText().getText()==null || panel.getDestinationFolderText().getText().length()==0){                    
                ...
                if (panel.getOverwriteCheckbox().isSelected()) args.add("-"+SplitParsedCommand.OVERWRITE_ARG);
                if (panel.getOutputCompressedCheck().isSelected()) args.add("-"+SplitParsedCommand.COMPRESSED_ARG); 
