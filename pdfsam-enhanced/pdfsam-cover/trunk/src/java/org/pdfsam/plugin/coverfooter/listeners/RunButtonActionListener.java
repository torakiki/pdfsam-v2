        ...
        try{
			...
			if(items == null || items.length == 0){		
				...
				JOptionPane.showMessageDialog(panel,
						GettextResource.gettext(Configuration.getInstance().getI18nResourceBundle(),"Select at least one cover or one footer"),
				...
				if((coveritems!=null && coveritems.length == 1)) {
					...
					String coverSelection = (coveritem.getPageSelection()!=null && coveritem.getPageSelection().length()>0)?coveritem.getPageSelection():CoverFooterMainGUI.ALL_STRING;
				...
				if((footeritems!=null && footeritems.length == 1)) {
					...
					String footerSelection = (footeritem.getPageSelection()!=null && footeritem.getPageSelection().length()>0)?footeritem.getPageSelection():CoverFooterMainGUI.ALL_STRING;
            	...
            	for (int i = 0; i < items.length; i++){
            		...
            		try{
						...
						item = items[i];
						String pageSelection = (item.getPageSelection()!=null && item.getPageSelection().length()>0)?item.getPageSelection():CoverFooterMainGUI.ALL_STRING;
					   ...
					   if(panel.getDestinationTextField().getText()==null || panel.getDestinationTextField().getText().length()==0){                    
						...
						if (panel.getDestinationTextField().getText().length() > 0){
							args1.add(panel.getDestinationTextField().getText()+File.separator+item.getInputFile().getName());
						}
