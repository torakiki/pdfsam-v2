 ...
 */
public class MergeMainGUI extends AbstractPlugIn{
...
//END_LAYOUT
    ...
    private void addTableRow(File file_to_add){
            if (file_to_add != null){
                ...
                try{
                    modello_merge_table.addRow(new MergeItemType(file_to_add.getName(),file_to_add.getAbsolutePath(), num_pages,MergeMainGUI.ALL_STRING,encrypt ));
	...
	private void addTableRowsFromNode(Node file_node){
            if (file_node != null){
				...
				try{				
                    modello_merge_table.addRow(new MergeItemType(file_to_add.getName(),file_to_add.getAbsolutePath(), num_pages,page_selection,encrypt ));
                    fireLogPropertyChanged(GettextResource.gettext(i18n_messages,"File selected: ")+file_to_add.getName(), LogPanel.LOG_INFO);
                }
                catch (Exception ex){
                    fireLogPropertyChanged(GettextResource.gettext(i18n_messages,"Error: ")+ex.getMessage(), LogPanel.LOG_ERROR); 
    ...
    private void setLayout(){
        ...
        spring_layout_merge_panel.putConstraint(SpringLayout.SOUTH, option_panel, 250, SpringLayout.NORTH, this);
        spring_layout_merge_panel.putConstraint(SpringLayout.EAST, option_panel, 0, SpringLayout.EAST, add_file_button);
        spring_layout_merge_panel.putConstraint(SpringLayout.NORTH, option_panel, 215, SpringLayout.NORTH, this);
		...
		try{
			if (arg0 != null){
				...
				for (int i = 0; i < merge_table.getModel().getRowCount(); i++){
					...
					file_node.addAttribute("name",(String) merge_table.getModel().getValueAt(i, MergeTableModel.PATH));
					file_node.addAttribute("pageselection",(String) merge_table.getModel().getValueAt(i, MergeTableModel.PAGESELECTION));
				}
