 ...
 * 
 */
public class MoveDownSelectionTableAction extends AbstractAction {
	...
	public MoveDownSelectionTableAction(JPdfSelectionTable mainTable) {
		super(GettextResource.gettext(Configuration.getInstance().getI18nResourceBundle(), "Move down"));
		...
		this.putValue(Action.SHORT_DESCRIPTION, GettextResource.gettext(Configuration.getInstance()
				.getI18nResourceBundle(), "Move down selected pdf file"));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (selectedRows.length > 0) {
			...
			if (selectedRows[selectedRows.length - 1] < (((AbstractPdfSelectionTableModel) mainTable.getModel())
					.getRowCount() - 1)) {
